...
use Plugin\Telegram\Services\LotteryService; // ✅ 新增
...
class Plugin extends AbstractPlugin
{
  ...
  protected array $commandConfigs = [
    ...
    // ✅ 新增：抽奖命令
    '/lottery' => ['description' => '抽奖说明与状态', 'handler' => 'handleLotteryCommand'],
    '/draw' => ['description' => '抽奖领取随机流量(1MB~1GB)', 'handler' => 'handleDrawCommand'],
  ];
  ...
  // ✅ 新增：/lottery 抽奖说明与状态
  public function handleLotteryCommand(object $msg): void
  {
    if (!$this->checkPrivateChat($msg)) return;

    $min = (int)$this->getConfig('lottery_min_mb', 1);
    $max = (int)$this->getConfig('lottery_max_mb', 1024);
    ...
    $svc = new LotteryService($this);
    $status = $svc->getTodayStatus((int)$user->id, (string)$msg->chat_id);

    $text .= "✅ 已绑定账号\n"
      . "• 今日已抽：{$status['user_used']} 次\n"
      . "• 今日剩余：{$status['user_left']} 次\n"
      . "• TG今日剩余：{$status['tg_left']} 次";

    $this->sendMessage($msg, $text);
  }

  // ✅ 新增：/draw 抽奖领流量
  public function handleDrawCommand(object $msg): void
  {
    if (!$this->checkPrivateChat($msg)) return;

    if (!(bool)$this->getConfig('enable_lottery', true)) {
      $this->sendMessage($msg, '抽奖功能未启用');
      return;
    }

    $requireBind = (bool)$this->getConfig('lottery_require_bind', true);
    $user = User::where('telegram_id', $msg->chat_id)->first();

    if (!$user && $requireBind) {
      $this->sendMessage($msg, "⚠️ 请先绑定账号。\n在网站复制订阅链接后发送：\n/bind <订阅链接>");
      return;
    }
    if (!$user) {
      $this->sendMessage($msg, "未绑定账号，无法发放流量");
      return;
    }

    $svc = new LotteryService($this);
    $res = $svc->drawForUser($user, (string)$msg->chat_id);

    $this->sendMessage($msg, $res['message']);
  }
