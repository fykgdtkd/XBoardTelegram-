...
随机领取 1MB~1GB 流量（范围可配置），自动发放到账号 `transfer_enable`。
...
| 配置项                         | 类型    | 默认值 | 说明 |
| ------------------------------ | ------- | ------ | ---- |
| `enable_lottery`               | boolean | true   | 启用抽奖 |
| `lottery_min_mb`               | number  | 1      | 最小奖励 MB |
| `lottery_max_mb`               | number  | 1024   | 最大奖励 MB（1024=1GB） |
| `lottery_daily_limit_per_user` | number  | 1      | 同站点账号每日次数 |
| `lottery_daily_limit_per_telegram` | number | 1   | 同 TG 账号每日次数 |
| `lottery_require_bind`         | boolean | true   | 是否必须绑定 |
